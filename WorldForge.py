...
class Supporters:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Supporters Page")
        pygame.display.set_icon(get_image("./logo.ico"))

        # Define other variables and data structures needed for your supporters list
        self.twitter_img = scale_images([get_image("./twitter.png")], (38,38))[0]
        self.discord_img = scale_images([get_image("./discord.png")], (64,64))[0]
        self.heart_img = scale_images([get_image("./heart.png")], (48,48))[0]
        self.supporters_list = [
            {"name": "brassedup(Scourge#1792)", "icon": self.discord_img, "quote": '"Who the fuck is John Madden anyway?\nHe doesnt even look like an Athlete!"'},
            {"name": "@HawkElusory", "icon": self.twitter_img, "quote": '"This editor got me thinking..."'},
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            ...
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            {"name": "Your Name/Handle!", "icon": self.heart_img, "quote": '"A quote from you!."'},
            # Add more supporters and their icons here
        ]
        ...
        self.page_size = 20  # Number of supporters to display per page
        self.current_page = 0  # The current page of supporters being displayed
        self.total_pages = len(self.supporters_list) // self.page_size + 1
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.screen.fill(THEMES[Chosen_Theme]['main'])

            # Draw the supporters list for the current page
            self.draw_supporters_list()
    ...
    def draw_hud(self):
        ...
        self.screen.blit(self.heart_img, (268, -5))
        
        # Display welcome message
        draw_text(
            self.screen,
            "./slkscr.ttf",
            f"The epic people that support me.",
            VECTOR2((830, 25)),
            size=20,
            color=THEMES[Chosen_Theme]['text'],
        )
    ...
    def draw_dropdown_menu(self):
        if self.dropdown_open:
            ...
            dropdown_x = 1405
            dropdown_y = 132 - (len(self.dropdown_items) * self.dropdown_item_height)
            dropdown_width = 180
            ...
            for i, item in enumerate(self.dropdown_items):
                self.dropdown_item_rects.append(pygame.Rect(
                    dropdown_x + 15,
    ...
    def draw_supporters_list(self):
        mouse_x, mouse_y = pygame.mouse.get_pos()
        # Calculate the starting and ending indices for the current page
        start_idx = self.current_page * self.page_size
        end_idx = min((self.current_page + 1) * self.page_size, len(self.supporters_list))

        # Calculate the number of supporters per row and column
        supporters_per_row = 4
        supporters_per_column = 4

        for i, supporter in enumerate(self.supporters_list[start_idx:end_idx]):
            # Calculate the row and column for each supporter
            row = i // supporters_per_row
            col = i % supporters_per_column

            # Calculate the position and dimensions for each supporter rectangle
            supporter_x = SUPPORTER_START_X + col * (SUPPORTER_WIDTH + SUPPORTER_MARGIN)
            supporter_y = SUPPORTER_START_Y + row * (SUPPORTER_HEIGHT + SUPPORTER_MARGIN)
            supporter_rect = pygame.Rect(
                supporter_x, supporter_y, SUPPORTER_WIDTH, SUPPORTER_HEIGHT
            )
            supporter_inner_rect = pygame.Rect(
                supporter_x + 8, supporter_y + 8, SUPPORTER_WIDTH - 16, SUPPORTER_HEIGHT - 16
            )

            # Draw the supporter rectangle
            pygame.draw.rect(self.screen, THEMES[Chosen_Theme]['accent'], supporter_rect, width=1)

            # Draw the supporter's name
            if not supporter_rect.collidepoint((mouse_x, mouse_y)):
                draw_text(
                    self.screen,
                    FONT,
                    supporter["name"],
                    (supporter_x + TEXT_MARGIN*3.6, supporter_y + TEXT_MARGIN*1.2),
                    DEFAULT_FONT_SIZE - 10,
                    THEMES[Chosen_Theme]['supporter color'],
                )
                pygame.draw.rect(self.screen, THEMES[Chosen_Theme]['accent'], supporter_inner_rect, width=1)

            if supporter_rect.collidepoint((mouse_x, mouse_y)):
                # Draw the supporter's icon
                if supporter["icon"] == self.heart_img:
                    icon_y = (supporter_y - 50) + ICON_MARGIN + 12
                    icon_x = (supporter_x - 100) + ICON_MARGIN*5.6
                if supporter["icon"] == self.discord_img:
                    icon_y = (supporter_y - 50) + ICON_MARGIN + 12
                    icon_x = (supporter_x - 100) + ICON_MARGIN*5.4
                if supporter["icon"] == self.twitter_img:
                    icon_y = (supporter_y - 50) + ICON_MARGIN + 26
                    icon_x = (supporter_x - 100) + ICON_MARGIN*5.7
                self.screen.blit(supporter["icon"], (icon_x, icon_y))
                pygame.draw.rect(self.screen, THEMES[Chosen_Theme]['supporter color'], supporter_inner_rect, width=2)
                draw_text(
                    self.screen,
                    FONT,
                    supporter["quote"],
                    (supporter_x + TEXT_MARGIN*3.6, supporter_y + TEXT_MARGIN*1.8),
                    DEFAULT_FONT_SIZE - 14,
                    THEMES[Chosen_Theme]['supporter color'],
                )

    def draw_navigation_arrows(self):
        # Draw left arrow if there are previous pages
        if self.current_page > 0:
            pygame.draw.polygon(
                self.screen,
                THEMES[Chosen_Theme]['arrow color'],
                [(ARROW_LEFT_X, ARROW_Y), (ARROW_LEFT_X + ARROW_SIZE, ARROW_Y - ARROW_SIZE // 2),
                 (ARROW_LEFT_X + ARROW_SIZE, ARROW_Y + ARROW_SIZE // 2)],
            )

        # Draw right arrow if there are more pages ahead
        if self.current_page < self.total_pages - 1:
            pygame.draw.polygon(
                self.screen,
                THEMES[Chosen_Theme]['arrow color'],
                [(ARROW_RIGHT_X, ARROW_Y), (ARROW_RIGHT_X - ARROW_SIZE, ARROW_Y - ARROW_SIZE // 2),
                 (ARROW_RIGHT_X - ARROW_SIZE, ARROW_Y + ARROW_SIZE // 2)],
            )

    def handle_click(self, pos):
        # Check if the left arrow is clicked to navigate to the previous page
        if self.current_page > 0 and pygame.Rect(
            ARROW_LEFT_X - ARROW_SIZE, ARROW_Y - ARROW_SIZE // 2,
            ARROW_SIZE * 2, ARROW_SIZE).collidepoint(pos):
            self.current_page -= 1

        # Check if the right arrow is clicked to navigate to the next page
        elif self.current_page < self.total_pages - 1 and pygame.Rect(
            ARROW_RIGHT_X - ARROW_SIZE, ARROW_Y - ARROW_SIZE // 2,
            ARROW_SIZE * 2, ARROW_SIZE).collidepoint(pos):
            self.current_page += 1
        ...
        for i, item in enumerate(self.dropdown_items):
            for i, item_rect in enumerate(self.dropdown_item_rects):
                if self.dropdown_rect.collidepoint(item_rect.center):
                    if item == 'Home':
...
class HomeScreen:
    ...
    def draw_recently_opened_files(self):
        ...
        draw_text(self.screen, "./slkscr.ttf", "Recent Files", VECTOR2((1245, 660)), color=THEMES[Chosen_Theme]['text'])
    ...
    def draw_image_display(self):
        ...
        if self.image_surface:
            if self.scale:
                ...
                self.image_surface = scale_images(
            ...
            draw_text(
                self.screen, "./slkscr.ttf", "Tileset", VECTOR2(450, 280), size=50, color=THEMES[Chosen_Theme]['text']
            )
            draw_text(
                self.screen,
                "./slkscr.ttf",
            ...
            draw_text(
                self.screen, "./slkscr.ttf", "Tileset", VECTOR2(400, 280), size=50, color=THEMES[Chosen_Theme]['text']
            )
            draw_text(
                self.screen,
                "./slkscr.ttf",
    ...
    def switch_to_supporters(self):
        self.running = False
        Supporters().run()
        print("switch to supporter screen")
...
class LevelEditor:
    ...
    def setup_assets(self, tileset, tileset_path, tilesize):
        if isinstance(tileset, type(None)):
            ...
            self.assets = {"tileset": cut_graphics(tileset, tilesize)}
            tile_list = list(self.assets)
            self.tile_list = tile_list
            return tile_list
    ...
    def updates(self):
        ...
        self.selected_tile_pos = (
            int(self.mouse_position[0] + self.scroll[0]) // self.tilemap.tile_size,
            int(self.mouse_position[1] + self.scroll[1]) // self.tilemap.tile_size,
        )
    ...
    def render(self, surface: PYSURFACE):
        ...
        if self.show_grid:
            for x in range(
                -int(self.scroll[0]) % self.tilemap.tile_size,
                surface.get_width(),
            ...
            for y in range(
                -int(self.scroll[1]) % self.tilemap.tile_size,
                surface.get_height(),
                self.tilemap.tile_size,
            ):
                pygame.draw.line(
                ...
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    if event.button == editor_controls["place tile"]:
                        ...
                        if pygame.key.get_mods() & pygame.KMOD_LSHIFT and self.ongrid:
                            if self.tile_id != self.tilemap.get_tile_at(self.selected_tile_pos):
                                self.tile_id = self.tilemap.get_tile_at(self.selected_tile_pos)
